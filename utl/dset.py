...
def getdatainfo(data_dir):
    ...
    for i,c in enumerate(SFOV_fns):
        CasecolSFOV=SFOV_fns[i][:SFOV_fns[i].find('V_S')+7]
        SFOV_add=(CasecolSFOV + '0.nii',CasecolSFOV + '1.nii', CasecolSFOV + '2.nii')
        ...
        LFOV_add=(SFOV_add[0].replace('SFOV','LFOV'),SFOV_add[1].replace('SFOV','LFOV'),SFOV_add[2].replace('SFOV','LFOV'))
        # LFOV_add=LFOV_add.replace('projc','projp')
        # field_add=field_add.replace('image','field')
        #print(field_add)
        #print(image_fns[i])

        datainfolist.append(DataInfoTuple(SFOV_add,CTD_add,LFOV_add))
    return datainfolist
